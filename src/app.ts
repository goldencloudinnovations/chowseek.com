...
const iosLink = document.querySelector<HTMLAnchorElement>('[data-app-ios]');
const androidLink = document.querySelector<HTMLAnchorElement>('[data-app-android]');
...
if (iosLink) {
  ...
  if (!appStoreReady) iosLink.addEventListener('click', (event) => event.preventDefault());
}
if (androidLink) {
  ...
  if (!playStoreReady) androidLink.addEventListener('click', (event) => event.preventDefault());
}
